/* desktop-files-backend.c generated by valac 0.24.0, the Vala compiler
...
void lxsession_db_default_apps_on_finish_scanning_installed (LxsessionDBDefaultApps* self) {
	...
	g_message ("desktop-files-backend.vala:189: Signal finish scanning with mode: %s", _tmp0_);
...
void lxsession_db_default_apps_on_finish_scanning_available (LxsessionDBDefaultApps* self) {
	...
	g_return_if_fail (self != NULL);
	_tmp0_ = self->priv->mode;
	g_message ("desktop-files-backend.vala:209: Signal finish scanning with mode: %s", _tmp0_);
	_tmp1_ = self->priv->mode;
	if (g_strcmp0 (_tmp1_, "display") == 0) {
		...
		_tmp3__length1 = default_available_length1;
		{
			...
			for (folder_it = 0; folder_it < _tmp3__length1; folder_it = folder_it + 1) {
				...
				folder = _tmp4_;
				{
					...
					const gchar* _tmp6_ = NULL;
					_tmp5_ = folder;
					g_message ("desktop-files-backend.vala:257: Scanning folder: %s", _tmp5_);
					_tmp6_ = folder;
					lxsession_db_default_apps_list_desktop_files (self, _tmp6_, "available", NULL, NULL);
					_g_free0 (folder);
				}
			}
		}
	} else {
		g_message ("desktop-files-backend.vala:263: No folders available set. Abort");
	}
	default_available = (_vala_array_free (default_available, default_available_length1, (GDestroyNotify) g_free), NULL);
}
...
static gboolean lxsession_db_default_apps_list_desktop_files_co (LxsessionDbDefaultAppsListDesktopFilesData* _data_) {
	...
	_state_0:
	g_message ("desktop-files-backend.vala:278: Start scanning\n");
	_data_->_tmp0_ = NULL;
				...
				for (_data_->info_it = _data_->info_collection; _data_->info_it != NULL; _data_->info_it = _data_->info_it->next) {
					...
					_data_->info = _data_->_tmp9_;
					{
						...
						__catch3_g_file_error:
						{
							...
							g_warning ("desktop-files-backend.vala:314: %s", _data_->_tmp25_);
	...
	g_message ("desktop-files-backend.vala:333: Finishing scanning\n");
	_g_object_unref0 (_data_->dir);
	if (_data_->_state_ == 0) {
...
void lxsession_db_default_apps_print_values_available (LxsessionDBDefaultApps* self) {
	...
	GList* _tmp3_ = NULL;
	g_return_if_fail (self != NULL);
	g_message ("desktop-files-backend.vala:854: Printing available webbrowser");
	_tmp0_ = self->webbrowser_available;
	{
		...
		for (entry_it = entry_collection; entry_it != NULL; entry_it = entry_it->next) {
			...
			entry = _tmp1_;
			{
				const gchar* _tmp2_ = NULL;
				_tmp2_ = entry;
				g_message ("desktop-files-backend.vala:857: %s", _tmp2_);
				_g_free0 (entry);
			}
		}
	}
	g_message ("desktop-files-backend.vala:860: Printing available email");
	_tmp3_ = self->email_available;
	{
		...
		for (entry_it = entry_collection; entry_it != NULL; entry_it = entry_it->next) {
			...
			entry = _tmp4_;
			{
				const gchar* _tmp5_ = NULL;
				_tmp5_ = entry;
				g_message ("desktop-files-backend.vala:863: %s", _tmp5_);
				_g_free0 (entry);
			}
		}
	}
...
void lxsession_db_default_apps_print_values_installed (LxsessionDBDefaultApps* self) {
	...
	GList* _tmp3_ = NULL;
	g_return_if_fail (self != NULL);
	g_message ("desktop-files-backend.vala:869: Printing installed webbrowser");
	_tmp0_ = self->webbrowser_installed;
	{
		...
		for (entry_it = entry_collection; entry_it != NULL; entry_it = entry_it->next) {
			...
			entry = _tmp1_;
			{
				const gchar* _tmp2_ = NULL;
				_tmp2_ = entry;
				g_message ("desktop-files-backend.vala:872: %s", _tmp2_);
				_g_free0 (entry);
			}
		}
	}
	g_message ("desktop-files-backend.vala:875: Printing installed email");
	_tmp3_ = self->email_installed;
	{
		...
		for (entry_it = entry_collection; entry_it != NULL; entry_it = entry_it->next) {
			...
			entry = _tmp4_;
			{
				const gchar* _tmp5_ = NULL;
				_tmp5_ = entry;
				g_message ("desktop-files-backend.vala:878: %s", _tmp5_);
				_g_free0 (entry);
			}
		}
	}
...
void lxsession_db_default_apps_save_keyfile (LxsessionDBDefaultApps* self) {
	...
	__catch9_g_file_error:
	{
		...
		g_warning ("desktop-files-backend.vala:947: %s", _tmp6_);
		_g_error_free0 (err);
	}
	__finally9:
	if (_inner_error_ != NULL) {
