...
void lxsession_db_default_apps_update_available (LxsessionDBDefaultApps* self) {
	...
	if (_tmp2_ != NULL) {
		...
		g_message ("desktop-files-backend.vala:271: No folders available set. Abort");
	}
	default_available = (_vala_array_free (default_available, default_available_length1, (GDestroyNotify) g_free), NULL);
}
...
static gboolean lxsession_db_default_apps_list_desktop_files_co (LxsessionDbDefaultAppsListDesktopFilesData* _data_) {
	...
	_state_0:
	g_message ("desktop-files-backend.vala:286: Start scanning\n");
	_data_->_tmp0_ = NULL;
				...
				for (_data_->info_it = _data_->info_collection; _data_->info_it != NULL; _data_->info_it = _data_->info_it->next) {
					...
					_data_->info = _data_->_tmp9_;
					{
						...
						__catch3_g_file_error:
						{
							...
							g_warning ("desktop-files-backend.vala:322: %s", _data_->_tmp25_);
	...
	g_message ("desktop-files-backend.vala:341: Finishing scanning\n");
	_g_object_unref0 (_data_->dir);
	if (_data_->_state_ == 0) {
...
void lxsession_db_default_apps_print_values_available (LxsessionDBDefaultApps* self) {
	...
	GList* _tmp3_ = NULL;
	g_return_if_fail (self != NULL);
	g_message ("desktop-files-backend.vala:862: Printing available webbrowser");
	_tmp0_ = self->webbrowser_available;
	{
		...
		for (entry_it = entry_collection; entry_it != NULL; entry_it = entry_it->next) {
			...
			entry = _tmp1_;
			{
				const gchar* _tmp2_ = NULL;
				_tmp2_ = entry;
				g_message ("desktop-files-backend.vala:865: %s", _tmp2_);
				_g_free0 (entry);
			}
		}
	}
	g_message ("desktop-files-backend.vala:868: Printing available email");
	_tmp3_ = self->email_available;
	{
		...
		for (entry_it = entry_collection; entry_it != NULL; entry_it = entry_it->next) {
			...
			entry = _tmp4_;
			{
				const gchar* _tmp5_ = NULL;
				_tmp5_ = entry;
				g_message ("desktop-files-backend.vala:871: %s", _tmp5_);
				_g_free0 (entry);
			}
		}
	}
...
void lxsession_db_default_apps_print_values_installed (LxsessionDBDefaultApps* self) {
	...
	GList* _tmp3_ = NULL;
	g_return_if_fail (self != NULL);
	g_message ("desktop-files-backend.vala:877: Printing installed webbrowser");
	_tmp0_ = self->webbrowser_installed;
	{
		...
		for (entry_it = entry_collection; entry_it != NULL; entry_it = entry_it->next) {
			...
			entry = _tmp1_;
			{
				const gchar* _tmp2_ = NULL;
				_tmp2_ = entry;
				g_message ("desktop-files-backend.vala:880: %s", _tmp2_);
				_g_free0 (entry);
			}
		}
	}
	g_message ("desktop-files-backend.vala:883: Printing installed email");
	_tmp3_ = self->email_installed;
	{
		...
		for (entry_it = entry_collection; entry_it != NULL; entry_it = entry_it->next) {
			...
			entry = _tmp4_;
			{
				const gchar* _tmp5_ = NULL;
				_tmp5_ = entry;
				g_message ("desktop-files-backend.vala:886: %s", _tmp5_);
				_g_free0 (entry);
			}
		}
	}
...
void lxsession_db_default_apps_save_keyfile (LxsessionDBDefaultApps* self) {
	...
	__catch9_g_file_error:
	{
		...
		g_warning ("desktop-files-backend.vala:955: %s", _tmp6_);
		_g_error_free0 (err);
	}
	__finally9:
	if (G_UNLIKELY (_inner_error_ != NULL)) {
