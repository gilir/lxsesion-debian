/* environement.c generated by valac 0.26.0, the Vala compiler
...
gboolean lxsession_lxsession_env_check_alone (LxsessionLxsessionEnv* self) {
	...
	g_message ("environement.vala:93: Getting lxsession pid");
	...
	g_message ("environement.vala:96: Checking pid : %s", _tmp3_);
	_tmp4_ = lxsession_pid;
	if (_tmp4_ == NULL) {
		g_message ("environement.vala:100: Lxsession not detected");
		result = TRUE;
		_g_free0 (lxsession_pid);
		return result;
	} else {
		g_message ("environement.vala:105: Lxsession detected");
		result = FALSE;
		_g_free0 (lxsession_pid);
		return result;
	}
...
"enu-xdg");
	...
	if (_tmp17_ == NULL) {
		...
		g_message ("environement.vala:149: confir_dirs is null, export : %s", _tmp20_);
	} else {
		...
		g_message ("environement.vala:176: custom_config :%s", _tmp43_);
		_tmp44_ = self->priv->config_dirs;
		g_message ("environement.vala:177: config_dirs :%s", _tmp44_);
		_tmp45_ = return_config;
		g_message ("environement.vala:178: confir_dirs not null, export : %s", _tmp45_);
		_g_free0 (custom_config_check);
	...
	if (_tmp46_ != NULL) {
		...
		g_message ("environement.vala:183: Exporting XDG_CONFIG_DIRS");
	...
	if (_tmp48_ == NULL) {
		...
		return_data = _tmp50_;
		_tmp51_ = return_data;
		g_message ("environement.vala:190: data_dirs is null, export : %s", _tmp51_);
	} else {
		...
		g_message ("environement.vala:217: custom_data :%s", _tmp74_);
		_tmp75_ = self->priv->data_dirs;
		g_message ("environement.vala:218: data_dirs :%s", _tmp75_);
		_tmp76_ = return_data;
		g_message ("environement.vala:219: data_dirs not null, export : %s", _tmp76_);
		_g_free0 (custom_data_check);
	...
	if (_tmp77_ != NULL) {
		...
		g_message ("environement.vala:224: Exporting XDG_DATA_DIRS");
		_tmp78_ = return_data;
...
" -remove _NET_CURRENT_DESKTOP");
	_tmp0_ = g_find_program_in_path ("dbus-launch");
	_g_free0 (dbus_path);
	dbus_path = _tmp0_;
	_tmp1_ = g_getenv ("DBUS_SESSION_BUS_ADDRESS");
	_tmp2_ = g_strdup (_tmp1_);
	_g_free0 (dbus_env);
	dbus_env = _tmp2_;
	_tmp3_ = dbus_path;
	if (_tmp3_ == NULL) {
		...
		_tmp4_ = dbus_env;
		if (_tmp4_ == NULL) {
			lxsession_lxsession_spawn_command_line_async ("dbus-launch --sh-syntax --exit-with-session");
		}
	}
	g_setenv ("SAL_USE_VCLPLUGIN", "gtk", TRUE);
	_tmp5_ = g_getenv ("QT_PLUGIN_PATH");
	_tmp6_ = g_strdup (_tmp5_);
	_g_free0 (qt_plugin);
	qt_plugin = _tmp6_;
	_tmp7_ = qt_plugin;
	if (_tmp7_ != NULL) {
		...
		if (g_strcmp0 (_tmp8_, "") != 0) {
			...
			gchar* _tmp11_ = NULL;
			_tmp9_ = qt_plugin;
			_tmp10_ = g_strconcat (_tmp9_, ":/usr/lib64/kde4/plugins:/usr/lib/kde4/plugins", NULL);
			_tmp11_ = _tmp10_;
			g_setenv ("QT_PLUGIN_PATH", _tmp11_, TRUE);
			_g_free0 (_tmp11_);
		}
	}
	_tmp12_ = lxsession_global_settings;
	_tmp13_ = lxsession_lxsession_config_get_item_string ((LxsessionLxsessionConfig*) _tmp12_, "Environment", "ubuntu_menuproxy", NULL);
	_tmp14_ = _tmp13_;
	_tmp15_ = g_strcmp0 (_tmp14_, "true") == 0;
	_g_free0 (_tmp14_);
	if (_tmp15_) {
